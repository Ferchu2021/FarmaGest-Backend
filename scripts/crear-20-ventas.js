/**
 * Script para crear 20 ventas de prueba para FarmaGest
 */
require("dotenv").config();
const db = require("../db");

console.log("🛒 Creando 20 ventas de prueba...\n");
...
function obtenerDatos(callback) {
  Promise.all([
    ...
    new Promise((resolve, reject) => {
      db.query(
        "SELECT producto_id, nombre, precio, stock FROM productos WHERE deleted_at IS NULL AND stock > 0 LIMIT 30",
        [],
        (err, result) => {
          if (err) return reject(err);
          const rows = result.rows || result || [];
          resolve(rows.map(p => ({
            ...
            nombre: p.nombre,
            precio: parseFloat(p.precio) || parseFloat(p.Precio) || 0,
...
function main() {
  obtenerDatos((err, datos) => {
    ...
    db.query(
      "SELECT numero_factura FROM ventas WHERE numero_factura IS NOT NULL ORDER BY numero_factura DESC LIMIT 1",
      [],
      (err, result) => {
        let ultimoNumero = 0;
        ...
        fechaBase.setDate(fechaBase.getDate() - 30); // Últimos 30 días
        ...
        for (let i = 0; i < 20; i++) {
          ...
          for (let j = 0; j < numItems; j++) {
            ...
            if (!productosUsados.has(producto.producto_id) && producto.stock > 0) {
          ...
          // Descuento aleatorio (0% a 25%)
          const descuentoPorcentaje = Math.floor(Math.random() * 26);
        ...
        console.log(`📦 Creando ${ventas.length} ventas...\n`);
        ...
        function procesarSiguiente() {
          ...
          crearVenta(venta, (err, ventaId) => {
            if (err) {
              ...
              console.log(`✅ [${index}/${ventas.length}] Venta creada: Factura ${venta.numero_factura} - $${venta.total.toFixed(2)} (${venta.items.length} items)`);
              creadas++;
            }
            procesarSiguiente();
          });
