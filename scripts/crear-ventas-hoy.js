/**
 * Script para crear 20 ventas con fechas de HOY para garantizar visibilidad
 */
require("dotenv").config();
const db = require("../db");

console.log("🛒 Creando 20 ventas con fechas de HOY para máxima visibilidad...\n");
  ...
  obtenerDatos((err, datos) => {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        // Generar 20 ventas con fechas de HOY (distribuidas a lo largo del día)
        const ventas = [];
        const hoy = new Date();
        ...
        for (let i = 0; i < 20; i++) {
          ...
          // Fecha de HOY con hora aleatoria (entre 8 y 20 horas)
        ...
        console.log(`📦 Creando ${ventas.length} ventas con fecha de HOY (${hoy.toLocaleDateString('es-AR')})...\n`);
        ...
        function procesarSiguiente() {
          if (index >= ventas.length) {
            ...
            console.log(`\n💡 Las ventas tienen fecha de HOY y deberían ser visibles inmediatamente`);
            console.log(`\n📋 Para verlas:`);
            console.log(`   1. Recarga la página de Ventas (Ctrl+F5)`);
            console.log(`   2. Haz clic en "Limpiar" para quitar filtros`);
            console.log(`   3. Las ventas aparecerán al inicio de la lista\n`);
            process.exit(0);
          }
          ...
          crearVenta(venta, (err, ventaId) => {
            if (err) {
              ...
              const fechaFormateada = new Date(venta.fecha_hora).toLocaleString('es-AR');
